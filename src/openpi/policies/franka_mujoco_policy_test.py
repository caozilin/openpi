...
def test_franka_mujoco_joint_tolerance_inputs_and_outputs() -> None:
    ...
    target = np.full((2, 3), 1.0, dtype=np.float32)
    frame = np.full((2, 3), 2.0, dtype=np.float32)
    tolerance = np.full((2, 3), 3.0, dtype=np.float32)
    result = transform(
        {
    ...
    assert result["actions"].shape == (2, 16)
    np.testing.assert_array_equal(result["actions"], np.concatenate((actions, target, frame, tolerance), axis=-1))

    padded = np.pad(result["actions"], ((0, 0), (0, 16)))
    ...
    np.testing.assert_array_equal(outputs["tolerance_frame"], frame)
